...
void main(){
	...
	outtextxy ( getmaxx()/2 -100 , getmaxy()-17  ,"Hill Climbing Algorithm" );
	...
	while(Open->length){
		if(Open->edges[Open->length]->h > Close->edges[Close->length]->h){
			outtextxy ( 250, 5  ,"Local Maximum" );
			delay(2000);
			backtrack();
			break;
		}
	...
	if(!Open->length && car.X_Start != car.X_Current && car.Y_Start != car.Y_Current){
		outtextxy ( 250, 5  ,"Local Maximum" );
		delay(2000);
		backtrack();
	}
