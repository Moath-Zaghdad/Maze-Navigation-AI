...
void PrintList(graph *list,char *name){
	...
	while(i <= list->length){
		fprintf(fp,"\n%d- previous %4hu, index %4hu, g %4hu, h %hu ,f(n) %hu\n ",
			i,
			list->edges[i]->previous, 
			list->edges[i]->index, 
			list->edges[i]->g, 
			list->edges[i]->h,
			list->edges[i]->g +	list->edges[i]->h);
		i++;
		fprintf(fp,"\n-------------------------------------------------------------------------------");
	}
...
void add_on_list( edge *NewEdge){
	...
	*/
	...
	DrawCheckedNode(NewEdge->index);
		...
		// pot it in the right plase "like it's sorted and you pot this puzel in it's blase"
		y = Open->length;
		for(x=Open->length-1 ; x > 0 && F(Open->edges[x]) < F(Open->edges[y]) ; x--){
			edge *z=calloc(1, sizeof(edge));
			z=Open->edges[y];
			Open->edges[y]=Open->edges[x];
			Open->edges[x] = z;
			y=x;
		}
